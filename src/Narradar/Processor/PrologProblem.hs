...
module Narradar.Processor.PrologProblem (
 ...
 ,RP, DRP, LP, DLRP
 ,skTransformWith, skTransformGoal, prologTRS'', addMissingPredicates, prepareProgram
 ,inferType
  ) where
...
data SKTransformProof = SKTransformNarrowingProof
                      | SKTransformInfinitaryProof
                      | SKTransformLabellingInfinitaryProof
  deriving (Eq, Show)

instance Pretty SKTransformProof where
  pPrint SKTransformInfinitaryProof = text "Termination of LP as termination of Infinitary Constructor Rewriting" $$
                                     text "(Schneider-Kamp transformation)"
  pPrint SKTransformNarrowingProof  = text "Termination of LP as termination of Narrowing proof" $$
                                     text "(Schneider-Kamp transformation)"
