...
addRules rr cr = foldr f cr rr where
...
instance (Foldable f, Ord id, HasId f id) => HasSignature (ClauseRules (Rule f v)) id
    where getSignature = getSignature . rules

instance (Foldable f, Ord id, HasId f id) => HasSignature [ClauseRules (Rule f v)] id
    where getSignature = getSignature . foldMap rules
...
instance (Ppr id, Ppr v) => Ppr (ClauseRules (RuleN id v)) where ppr = vcat . map ppr . rules


type PrologRules id v = Map (Prolog.Clause'' (WithoutPrologId id) (TermN (WithoutPrologId id) v))
                            (ClauseRules (RuleN id v))

prologTRS'' :: (RemovePrologId id, Ord id, Ord v) => PrologRules id v -> Signature id -> NarradarTRS id v
...
--    max_depth = Prelude.pred $ getMax $ foldMap3 (Max . termDepth) rr
    max_depth = Prelude.pred $ getMax $ foldMap2 (Max . termDepth) (lhs <$$> db)
...
applyAF :: AF.ApplyAF t id => t -> PrologM id v t
...
traceUpdatesM tit m = do
   ...
   when (not (null added && null deleted)) $ do
     db  <- getDB
     traceM (vcat ([text "+" <+> ppr r <+> parens (ppr k)
                      | ((Pred p _ :- _, tt), p_rr) <- Map.toList db
                      , let k = Pred p tt
                      , r <- rules p_rr
                      , r `elem` added ]
                 ++ [text "-" <+> ppr r <+> parens (ppr k)
                      | ((Pred p _ :- _, tt), p_rr) <- Map.toList db
                      , let k = Pred p tt
                      , r <- rules p_rr
                      , r `elem` deleted ]))
...
skTransform :: (Ord id, Ord v, Ppr id) => [Prolog.Clause'' id (TermN id v)] -> PrologRules (PrologId id) v
...
labellingConsTrans (g,mm) pgm = Set.singleton $ runIdentity $ do
 ...
 where
  ...
  goalAF  = AF.mapSymbols Plain (skTransformAF pgm (mkGoalAF (g,mm)))
...
invariantEVPreds heuristic f (the_m, af, sig, lrr)
  ...
  = do (pf, i) <- embed $ runHeu heuristic af r pos
       ...
       pprTrace (hsep$ punctuate comma $ [text "pred:"   <+> ppr (Prolog.pred $ cHead c)
                                         ,text "symbol:" <+> ppr pf
                                         ,text "i:"      <+> int i
                                         ,text "pos:"    <+> ppr pos
...
invariantEVCons f = do
  ...
  case extra of
   ((k,rule@(l:->r),pos):_) ->  do
     ...
     traceM (hsep $ punctuate comma $ [text "pred:"   <+> ppr (Prolog.pred $ cHead $ fst k)
...
--    lift updateAF
  ...
  traceUpdatesM (text "Adding clauses for calls" <+> ppr pp) $  mapM (uncurry appendRR) things
  kk' <- getKK
  rr' <- getRR
...
{-
       ...
       traceUpdatesM (text "deleting irrelevant patterns" <+> ppr ((args . cHead) <$> irrelevant)) $
        modifyRR' (\x -> foldr Map.delete x irrelevant)
-}
       cutAF f i
  ...
  where
    ...
    r'     = updateAt (init pos) (mapRootSymbol (const f')) r
    f'     = mapLabel (fmap (delete i)) f


-- fixOutputPatterns k rule pos r' | pprTrace (text "fixOutputPatterns" <+> ppr r') False = undefined
fixOutputPatterns  k pos r' = traceUpdatesM (text "Fixing success patterns for" <+> ppr r' <+> text "at pos" <+> ppr pos) $ fixOutputPatterns' k pos r'
--fixOutputPatterns' k pos r | pprTrace (text "Fix output patterns for" <+> ppr r <+> text "at pos" <+> ppr pos) False = undefined
...
getInstantiationMappings pos goal = do {res <- getInstantiationMappings' pos goal; traceM (text "getInstantiationMappings" <+> ppr goal <+> ppr pos <+> equals <+> ppr res); return res}
getInstantiationMappings' pos goal = do
    ...
    pats <- runListT $ do
        ...
        tt <- ListT $ return $
              if Prelude.any isLabelledDown (directSubterms goal)
                      then filter (Prelude.any isLabelledDown . foldMap subterms) all_tt
...
deriveRulesForCall pos@(viewLast -> (prefix,i)) call = do
  traceM (text "deriveRulesForCall" <+> ppr call <+> ppr pos)
  max_d <- getMaxDepth
...
{-
      when (isVar v) $ do
             ...
             lift$ pprTrace (text "deriveRulesForCall" <+> ppr call <+> text "-->" <+> text "fixOutputPatterns" <+> ppr (applySubst sigma $ rhs rule2)) $
...
deriveCallsForOutput k out pos = do
  traceM (text "deriveCallsForOutput" <+> ppr out <+> parens (text "depth:" <+> int(termDepth out))  <+>text "at" <+> ppr pos)
  max_d <- getMaxDepth
...
--copyLabelling t u | pprTrace (text "copyLabelling" <+> ppr t <+> ppr u) False = undefined
...
traceUpdates tit rr rr' =
    pprTrace (tit $$
              vcat ([text "+" <+> ppr r | r <- toList added]
                 ++ [text "-" <+> ppr r | r <- toList deleted])) rr'
  where
