...
webSolver      = mainSolver (ProgressT . aproveWebProc)

-- solver that uses a cmd line version of Aprove, use only for testing
localSolver       = localSolver' "/usr/local/lib/aprove/runme"
localSolver' path = mainSolver (ProgressT . aproveProc path)

-- solver that uses an Aprove server running on the host machine
srvSolver = mainSolver (ProgressT . aproveSrvProc)

-- Attempts to connect to the Aprove local server, falls back to the Aprove web server
--srvWebSolver = 

-- Our main solving scheme
mainSolver aproveProc = (cycleProcessor >=> afProcessor >||> aproveProc) `refineBy` narrowingProcessor

-- For debugging purposes
mainSolverPure = (cycleProcessor >=> afProcessor) `refineBy` narrowingProcessor

refineBy :: (Prelude.Monad m, Bind m' m m, MPlus m m m) => (a -> m a) -> (a -> m' a) -> a -> m a
refineBy step refiner x = iterateMn 4 (refiner >=> step) (x >>= step)
