...
lookup id (AF map)    = maybe (fail "not found") return (Map.lookup id map)
...
instance Monoid AF where
  mempty  = AF Map.empty
  mappend (AF m1) (AF m2) = AF$ Map.unionWith mappend m1 m2

initAF t | sig <- getSignature t = fromList [ (d, [0.. getArity sig d -1]) | d <- Foldable.toList(definedSymbols sig `mappend` constructorSymbols sig)]